...
function MetricMini({ icon: Icon, label, value, change }: { icon: typeof MapPin; label: string; value: string; change: string }) {
  return (
    <div className="rounded-xl bg-card/80 backdrop-blur-sm p-4 space-y-1.5 border border-border/30 hover:border-primary/20 hover:shadow-soft transition-all duration-300 group">
...
export function Hero() {
  return (
    <section className="pt-36 pb-28 md:pt-48 md:pb-40 bg-background relative overflow-hidden noise-overlay">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(hsl(var(--border)/0.15)_1px,transparent_1px),linear-gradient(90deg,hsl(var(--border)/0.15)_1px,transparent_1px)] bg-[size:72px_72px] [mask-image:radial-gradient(ellipse_50%_50%_at_50%_50%,black,transparent)]" />
      ...
      <div className="container grid lg:grid-cols-2 gap-16 lg:gap-24 items-center relative z-10">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl lg:text-[3.75rem] font-heading font-extrabold leading-[1.06] text-foreground tracking-tight">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
            <Button
              size="lg"
              asChild
              className="text-base px-8 rounded-full shadow-medium hover:shadow-lg transition-all duration-300 hover:scale-[1.03] active:scale-[0.97] bg-primary hover:bg-primary/90 relative overflow-hidden group animate-glow-pulse"
            >
              <Link to="/pricing">
        ...
        >
          <div className="absolute -inset-8 bg-gradient-to-br from-primary/12 via-accent/6 to-primary/4 rounded-[2.5rem] blur-3xl animate-gradient-shift" />
